...
class DBHelper {
  static createDBTables() async {
    ...
    await createDBTable<Product>(fromJson:Product.fromJson, structure : LocalDatabaseStructure.productStructure);
    await createDBTable<ProductUnit>(fromJson:ProductUnit.fromJson, structure : LocalDatabaseStructure.productUnitStructure);
    await createDBTable<PosSession>(fromJson:PosSession.fromJson, structure : LocalDatabaseStructure.posSessionStructure);
    await createDBTable<AccountTax>(fromJson:AccountTax.fromJson, structure : LocalDatabaseStructure.accountTaxStructure);
    await createDBTable<SaleOrderInvoice>(fromJson:SaleOrderInvoice.fromJson, structure : LocalDatabaseStructure.saleOrderStructure);
    await createDBTable<SaleOrderLine>(fromJson:SaleOrderInvoice.fromJson, structure : LocalDatabaseStructure.saleOrderLineStructure);
    await createDBTable<ConnectedPrinter>(fromJson:ConnectedPrinter.fromJson, structure : LocalDatabaseStructure.appConnectedPrinters);
    await createDBTable<AccountJournal>(fromJson:AccountJournal.fromJson, structure : LocalDatabaseStructure.accountJournalStructure);
    await createDBTable<User>(fromJson:User.fromJson, structure : LocalDatabaseStructure.userStructure);
  }
  static dropDBData({isDeleteBasicData = false}) async {
    ...
    await deleteTableRows<SaleOrderInvoice>(fromJson:SaleOrderInvoice.fromJson);
    await deleteTableRows<AccountJournal>(fromJson:AccountJournal.fromJson);
    await deleteTableRows<ConnectedPrinter>(fromJson:ConnectedPrinter.fromJson);
  }

  static dropDBTable({isDeleteBasicData = false}) async {
    await dropTable<RemoteSupportTicket>(fromJson:RemoteSupportTicket.fromJson);
  ...
  static deleteFile() async {
    final io.Directory directory = await path_provider.getApplicationSupportDirectory();
  ...
  static Future<void> createDBTable<T>({required fromJson , required String structure}) async {
    try {
      GeneralLocalDB? generalLocalDBInstance = GeneralLocalDB.getInstance<T>(fromJsonFun: fromJson);
      await generalLocalDBInstance!.createTable(structure: structure);
    } catch (e) {
      handleException(
          exception: e,
          navigation: false,
          methodName: "createDBTable");
    }
  }
